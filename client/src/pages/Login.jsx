...
const Login = () => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    if (loginType === "login") {
      ...
      if (response && response.success && response.token) {
        // Store the token and OTP for verification
        setSignupToken(response.token);
        setGeneratedOtp(response.otp); // Store the OTP from backend
        setOtpStep(true);
      }
    }
  };
      ...
      {otpStep ? (
        ...
        <>
          ...
          <div className="w-80 p-4 bg-green-50 border border-green-200 rounded-lg">
            <p className="text-sm text-green-800 font-semibold mb-2 text-center">
              Your OTP Code:
            </p>
            <div className="text-2xl font-mono font-bold text-green-600 tracking-widest text-center">
              {generatedOtp}
            </div>
          ...
          <div className="flex flex-col gap-4 w-80">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-2 text-center">
                Copy the OTP from above and enter it here
              </p>
            </div>
        ...
        // Login/Signup Form Section (Same as before)
        <>
          <div className="text-center">
            <h2 className="text-xl font-bold text-gray-800">
              {loginType === "login" ? "Welcome Back" : "Create Account"}
